...
fn main() {
    ...
    input_string.pop();
    println!("{:?}", input_string.split(',').map(|word| { 
        word.chars().fold(0, |hash, sym| {
            next_hash(hash, sym)
        })
        }).sum::<usize>());
    ...
    input_string.split(',')
        .for_each(|word| { 
            let (matched,[op_label, op]) = re.captures(word).unwrap().extract();
            ...
            match op{
                ...
                digit_str => {
                    ...
                    if let Some(lens_box)= boxes.get_mut(&box_nr){
                        if let Some(pos) = lens_box.iter_mut().position(|(lens_label, _)| lens_label == op_label){
    ...
    println!("{}", (0..256).map(|box_nr| {
        (box_nr +1)*boxes.get(&box_nr).map_or(0, |lens_box| lens_box.iter().enumerate().fold(0, |sum,(pos,(_, focal_length))| sum+(pos+1)*focal_length))
    }).sum::<usize>());
}
