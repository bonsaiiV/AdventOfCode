...
fn main() {
    ...
    let result1: usize = hands_vec
        .iter()
        .enumerate()
        .map(|(index, (_, points))| (index + 1) * points)
        .sum();
    println!("{}", result1);
}
...
fn _get_score(hand: &String) -> usize {
    ...
    hand.chars().fold(&mut counts, |counts_f, next_char| {
        counts_f[to_index(next_char)] += 1;
        counts_f
    });
    ...
    if counts.contains(&5) {
        hand_score.insert(0, 6)
    } else if counts.contains(&4) {
        hand_score.insert(0, 5)
    } else if counts.contains(&3) && counts.contains(&2) {
        hand_score.insert(0, 4)
    } else if counts.contains(&3) {
        hand_score.insert(0, 3)
    } else if counts.iter().filter(|c| **c == 2).count() == 2 {
        hand_score.insert(0, 2)
    } else if counts.contains(&2) {
        hand_score.insert(0, 1)
    } else {
        hand_score.insert(0, 0)
    }
    let score_of_hand = hand_score
        .iter()
        .fold(0, |score, next_part| score * 100 + next_part);
...
fn _get_score2(hand: &String) -> usize {
    ...
    counts.sort();
    let score_of_hand = get_hand_type((
        count_of_joker + counts.pop().unwrap(),
        counts.pop().unwrap(),
    ));
    hand_score
        .iter()
        .fold(score_of_hand, |score, next_part| score * 100 + next_part)
}
